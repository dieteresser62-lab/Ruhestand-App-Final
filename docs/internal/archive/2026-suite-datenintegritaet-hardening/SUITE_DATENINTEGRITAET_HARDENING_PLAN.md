...
| 16 | [SLICE_SUITE_DATA_16_GESAMTINTEGRATION.md](./SLICE_SUITE_DATA_16_GESAMTINTEGRATION.md) | Gesamtintegration, Browser, Evidenz und Doku | P1/P2 | 1-15 | 3 | Review-Blocker S16-1 bis S16-10 technisch nachgebessert - Re-Review ausstehend |
...
### Technischer Umsetzungsstand vom 2026-07-29

- `SuiteDataIntegrityTraceabilityV1` ordnet O-01 bis O-22 konkreten
  Testdateien, Witness-Markern und Node-/Browsergates zu. Der fokussierte
  Integrationscontract bestand nach der Review-Nachbesserung mit 358/358
  Assertions.
- Das Browsergate fuehrt 27/27 Flows aus. Neu belegt sind die reale
  3-Bucket-Bear-Diagnose, Hybridprofil-Blockade, versionierte
  Sweep-Provenienz und der Evaluate-/Apply-Fingerprint des experimentellen
  Optimizers; Preview/Commit und Import/Recovery bleiben Teil desselben Gates.
- Die Traceability-Matrix deckt alle neun sichtbaren Sweep- und alle acht
  interaktiven Optimizerparameter ab. Sie belegt kanonische Zuordnung,
  Consumer und Provenienz, nicht eigenstaendig KPI-Wirkung.
  `sweepMaxBearRefillPct` bleibt als sichtbare Basisannahme in UI,
  Achsenauswahl und Persistenz erhalten. Nur `sweepHorizonYears` wurde
  entfernt, weil die Browseroberflaeche den Horizont aktuarisch bestimmt; der
  explizite programmatische Requestvertrag bleibt erhalten.
- Die Review-Nachbesserung gleicht reale Sweep-Range-Inputs generisch und
  fail-closed mit der Matrix ab, prueft Witness-Gates gegen
  `TEST_EXECUTION_POLICY`, synchronisiert das UI-Orchestrierungs-Mock exakt
  mit `Simulator.html` und entfernt einen verwaisten Horizon-Storagekey.
- `npm run build:engine` erzeugte den Modul-Fallback bytegleich; der
  `engine.js`-Blob blieb `ec49961ed58134928723be5ddb6185e11f7fbcb5`.
  Backtest-, Monte-Carlo- und FlowDelta-Baselines blieben ebenfalls
  unveraendert.
- Abschlussgates nach der Review-Nachbesserung: `npm test` 9.184/9.184
  Assertions in 137 entdeckten Testdateien bei 0 offenen Handles; Browser
  27/27; Coverage 77,89% (38.479/49.403 V8-Zeilenbereiche in 207 Dateien);
  Doku-Evidenz 69 MKT,
  55 FOR, 17 MAP und 10 Modellmatrixtermine. Build, Syntax-, JSON-, Scope-
  und Diffchecks sind gruen.
- Coverage bleibt eine Risikomessung: `simulator-sweep.js` erreicht durch den
  neuen DOM-Vertrag im Node-V8-Report 6,22%; Playwright-Anteile bleiben
  ausserhalb der Messung.
- Exakt drei Programmdateien wurden geaendert; Engine-Semantik,
  `engine.js`, `workers/`, `dist/`, `src-tauri/` und Releaseartefakte sind
  unveraendert. Unabhaengiges Review, Commit und Push stehen aus.
...
| 2026-07-28 | Slice 16 durch Codex technisch umgesetzt | `SuiteDataIntegrityTraceabilityV1` bindet O-01 bis O-22, sechs Browservertraege, vier Paritaetsachsen, alle sichtbaren Sweep-/Optimizerparameter und unveraenderte Delta-Baselines. Zwei interaktive Sweep-No-ops (`sweepHorizonYears`, `sweepMaxBearRefillPct`) wurden aus UI/Achsenauswahl/Persistenz entfernt; programmatische Requestvertraege und Engine-Semantik bleiben erhalten. Fokussiert 292/292, `npm test` 9.115/9.115 bei 0 offenen Handles, Browser 27/27, Coverage 77,84%, Engine-Build und Doku-Evidenz gruen. Exakt drei Programmdateien; unabhaengiges Review, Commit und Push ausstehend. |
| 2026-07-28 | Slice 16 durch Claude reviewt | Status blockiert. Gates unabhaengig reproduziert: fokussiert 292/292, `npm test` 9.115/9.115 bei 0 offenen Handles, Browser 27/27, `npm run docs:evidence` gruen mit 10 Matrix-Reviewterminen; Diff exakt 16 Dateien. Blocker S16-1: Die Entfernung der zwei Sweep-Felder aendert die effektive Engine-Semantik. `buildSweepInputs` schreibt jeden Schluessel aus `caseOverrides` in die Inputs, auch wenn `params` ihn nicht mehr liefert; gemessen wird `maxBearRefillPctOfEq` dadurch `undefined`, `engine/core.mjs:81` normalisiert das auf 0, und `computeCappedRefill` quittiert 0 Prozent mit `bedarf: 0` und der Diagnose "Aktion unterdrueckt" (bei 5 Prozent: 25.000 Euro Cap). Der interaktive Sweep rechnet damit ohne Baerenmarkt-Auffuellung, waehrend der Hauptlauf den Formularwert nutzt - entgegen der Zusage, keine Engine-Semantik zu aendern. Ein KPI-Delta habe ich in fuenf Szenarien mal vier Refill-Werten nicht nachgewiesen; der Mechanismus ist belegt. Die Korrektur liegt in `app/simulator/sweep-runner.js` und braucht eine vierte Programmdatei (Nutzerentscheidung). Blocker S16-2: AK-8 verletzt, `docs/reference/ARCHITEKTUR_UND_FACHKONZEPT.md:2535` fuehrt das entfernte `sweepMaxBearRefillPct` weiter als konfigurierbaren Sweep-Parameter und laesst zwei sichtbare Felder aus. Restrisiken S16-3 (die Sweep-Haelfte des Contract-Tests ist nicht fail-closed: ein neu hinzugefuegtes `id="sweepInflationPct"` wird von der fest verdrahteten Regex nicht erfasst, der Optimizer-Teil ist ueber die reale Registry dagegen fail-closed), S16-4 (Gate-Zugehoerigkeit wird nur gegen eine Zweiwerteliste geprueft, nicht gegen `TEST_EXECUTION_POLICY`; die sechs Browservertraege haben kein `gate`-Feld), S16-5 (asymmetrischer Massstab: geprueft wird Durchreichung, nicht KPI-Wirkung - den Test haette das entfernte Feld bestanden), S16-6 (README nennt "acht nachgewiesen wirksame Dimensionen"), S16-7 (Mock-DOM in `simulator-ui-orchestration.test.mjs` fuehrt die entfernten Felder weiter), S16-8 (der reale Sweep-Pfad ohne die zwei Schluessel ist ungetestet - deshalb ging S16-1 durch 9.115 gruene Assertions), S16-9 (verwaiste `sim.sweep.*`-Schluessel) und S16-10 (Zeilenangabe in `tests/README.md`). Geprueft und verworfen: Achsenpersistenz, Marker in Kommentaren, Dynamic-Flex-Bruch ohne `horizonYears` (aktuarisch aufgeloest, KPIs identisch), Main-/Worker-Divergenz und die sechs Delta-Baselines. |
| 2026-07-29 | Slice 16 Review-Blocker durch Codex technisch nachgebessert | S16-1 ohne vierte Produktivdatei geschlossen: `sweepMaxBearRefillPct` ist in den drei vorhandenen UI-/Orchestrierungsdateien wiederhergestellt und der reale Browserlauf prueft den Wert 5 in der Ergebnisprovenienz; nur der aktuarisch wirkungslose direkte Horizont bleibt entfernt. S16-2 synchronisiert das Fachkonzept mit allen neun sichtbaren Dimensionen. S16-3 bis S16-10 sind gekoppelt behandelt: generische fail-closed HTML-Inventarisierung, Gate-Abgleich gegen `TEST_EXECUTION_POLICY`, Status `canonical_path_verified` statt KPI-Wirkungsbehauptung, Dokuabgrenzung, exakter Real-UI-/Mock-Abgleich, Browserprovenienz, Horizon-Storagebereinigung und korrigierte Testzeilenangabe. Exakt drei Produktivdateien. Fokussiert 358/358, `npm test` und Coverage je 9.184/9.184 Assertions bei 0 offenen Handles, Browser 27/27, Coverage 77,89%, Engine-Build, Doku-Evidenz, Syntax-, JSON- und Diffchecks gruen; unabhaengiges Re-Review ausstehend. |
| 2026-07-29 | Slice 16 Nachbesserung durch Claude re-reviewt | Status freigegeben; keine Blocker. S16-1 bis S16-10 geschlossen. S16-1 wurde an der Ursache behoben: `sweepMaxBearRefillPct` ist in `Simulator.html`, Achsenauswahl, UI-Bindings und Persistenz wiederhergestellt, gemessen liefert `buildSweepInputs` statt `undefined` wieder den sichtbaren Feldwert, und der Playwright-Sweep prueft den Wert 5 bis in `SweepExecutionV2.results[0].params.maxBearRefillPct` - damit existiert der End-to-End-Witness, an dem der Fehler zuvor vorbeigekommen war (schliesst zugleich S16-8). S16-2: das Fachkonzept fuehrt alle neun sichtbaren Dimensionen und grenzt Datenpfadnachweis von KPI-Wirkung ab. S16-3 gegengeprueft: ein zusaetzliches `id="sweepInflationPct"` im realen Fieldset wird erkannt, veraendertes Legend-Markup laesst die Assertion fallen. S16-4: das Gate wird aus `getTestExecutionPolicy` abgeleitet, 21 Witness-Dateien ohne Widerspruch. S16-5 bis S16-7, S16-9 und S16-10 ebenfalls in der Sache erledigt. Gates unabhaengig reproduziert: fokussiert 358/358 und 59/59, `npm test` 9.184/9.184 bei 0 offenen Handles, Browser 27/27 ohne Fehlermeldung, `docs:evidence` gruen, Scope-Audit deckungsgleich mit 18 Dateien, verbotene Bereiche unveraendert. Neue Restrisiken T16-1 (der `undefined`-Override auf `horizonYears` besteht fort - gemessen `base=27 -> undefined` -, waehrend die Nachbesserungstabelle ihn pauschal als beseitigt darstellt; belegt folgenlos, weil der Horizont aktuarisch aufgeloest wird, aber programmatische Teilaufrufe verlieren ihre Basisannahmen), T16-2 (die Fail-closed-Erkennung sieht nur `<input>` innerhalb des Sweep-Ranges-Fieldsets; ein Feld ausserhalb und ein `<select>` darin bleiben gemessen unsichtbar), T16-3 (`getTestExecutionPolicy` liefert fuer unbekannte Dateinamen still `in-process`, eine nicht entdeckte Witness-Datei bestuende die Gate-Pruefung als Node-Witness), T16-4 (die unbedingte Raeumung von `sim.sweep.horizonYears` loescht den Wert bei jedem Laden, falls das Feld je zurueckkehrt; der neue Regressionstest zementiert das Verhalten), T16-5 (die Beispiel-Range fuer `maxBearRefillPct` wurde im Fachkonzept beilaeufig von 30-60 auf 0-5 Prozent geaendert, ohne Entscheidungseintrag), T16-6 (Statusvokabular nur per `startsWith` geprueft) und T16-7 (die neue Coveragezahl 6,22 Prozent fuer `simulator-sweep.js` ist leicht als Regression fehlzulesen). Geprueft und verworfen: Wurfrisiko der Persistenzraeumung, Rekursion durch den `run-tests.mjs`-Import, verwaiste Achsenoptionen, Scope-Audit-Abweichung und die sechs Delta-Baselines. |
| 2026-07-29 | Slice-uebergreifendes Gesamtreview durch Claude | Status blockiert bezogen auf die Feststellung, dass das Gesamtvorhaben abgeschlossen ist; die sechzehn Einzelfreigaben bleiben unberuehrt. Gates am Kopfcommit `860ba50` unabhaengig reproduziert: `npm test` 9.184/9.184 bei 0 offenen Handles, Browser 27/27 ohne Fehlerzeile, `npm run docs:evidence` gruen, Arbeitsbaum sauber. Drei Blocker. G-01: Achtunddreissig der fuenfundsechzig Diagnosebefunde nennen Slice 16 als Nachweis-Slice; das Slice-16-Dokument nennt keinen einzigen dieser Bezeichner und besitzt keinen Traceability-Abschnitt. Sechs davon (OPT-07, IMP-02, PER-02, PER-05, QA-01, QA-02) erscheinen in gar keinem Slice-Dokument. G-02: Die acht verbindlichen Zielvertraege I-01 bis I-08 kommen im gesamten Repository nur in ihrer eigenen Definitionsstelle vor - null Vorkommen in Slices, Tests und Inventar. G-03: Mit eingefrorener Systemuhr ist die Suite am 2026-12-31 noch gruen und faellt ab dem 2027-01-01 aus - `tests/balance-ui-orchestration.test.mjs:1186` (155/155 statt 238/238, Exitcode 1), weil der Test `marketCsvTargetYear` fest auf 2025 setzt, waehrend `resolveCurrentMarketCsvTargetYear` ohne `pendingCommit` auf `deriveCompletedCalendarYear(new Date())` zurueckfaellt; `tests/browser-smoke.test.mjs:721-726` enthaelt dieselbe Konstruktion. Restrisiken G-04 (Goldzielanteil hat drei Domaenen - Profil 0..100 fail-closed, Aggregation unbegrenzt, `balance-reader.js:301` setzt ueber 50 still auf 7,5; gemessen wird ein Haushaltsziel von 600.000 EUR zu 75.000 EUR, interaktiv erreichbar ueber `depot-tranchen-manager.html:736` mit `max="100"`), G-05 (fail-open im Rechenkern: `computeHouseholdFlexFactor` liefert vollstaendig 0,875, bei fehlendem `hasPartner` 0,5, bei `hasPartner: "true"` als String ebenfalls 0,5 - dieselbe Fehlerklasse, gegen die O-16 den Importpfad haertet - und bei fehlenden Feldern 0, jeweils ohne Meldung), G-06 (`buildSweepInputs` setzt bei einem Aufruf mit einem Parameter acht von neun Basiswerten auf `undefined`, nicht nur `horizonYears` wie in T16-1 beschrieben; `tests/longevity-engine-runner.test.mjs:200` ist ein bereits existierender Teilaufrufer und bleibt gruen, der Contract-Test von Slice 16 kann den Fall strukturell nicht erzeugen), G-07 (`app/shared/persistence-key-policy.js` fuehrt eine zweite, durch einen Importzyklus erzwungene Fassung des Schluesselvertrags ohne bindenden Test; Verdacht auf Datenverlust beim Vollbackup-Import gemessen widerlegt), G-08 (das Nachweisinventar kennt die Slices 14 und 15 nicht - vertragskonform zu AK 1, aber eine Luecke im Endzustand), G-09 (230 Befundkennungen in den Slice-Dokumenten, davon mindestens 92 als offen ausgewiesen und 45 im Hauptplan nirgends genannt; kein zentrales Register offener Punkte) und G-10 (lokales `dist/` vom 2026-07-22, gitignoriert, wird von `scripts/build-tauri.ps1:293` beim Bauen erneuert). Geprueft und verworfen: Fail-open-Runner, stille Assertionsverluste ueber die Zeit, Zerstoerung nicht gelisteter Schluessel beim Import, stiller Ausschluss beim Backup-Export, Divergenz in `profile-key-policy`, veraltetes `engine.js` und nicht registrierte Testdateien. Vollstaendiges Review in `docs/internal/SUITE_DATENINTEGRITAET_GESAMTREVIEW.md`. |
| 2026-07-29 | Gesamtreview-Blocker G-01 bis G-03 durch Codex technisch nachgebessert | G-01: `SuiteDataIntegrityTraceabilityV1` fuehrt jetzt alle 65 Planfindings mit Fix-/Nachweisslices und aufloesbaren Orakeln beziehungsweise direkten Witnesses; der Contract liest Findings-Register und Traceability-Tabelle direkt aus diesem Hauptplan und assertiert die exakte 38er-Slice-16-Teilmenge. G-02: I-01 bis I-08 sind mit exaktem Plantitel und Oracle-/Witness-/Gate-Kette inventarisiert und fail-closed geprueft. G-03: Node- und Browser-CSV-Roundtrip leiten Zieljahr, Stichtag, Dateiname, Reihen und Erwartungen aus dem im jeweiligen Laufzeitkontext abgeschlossenen Kalenderjahr ab; der Node-Test ist mit eingefrorener Uhr am 2030-05-05 gruen. Keine Produktiv- oder Engine-Semantik geaendert. Fokussiert 1.066/1.066 und 238/238, Zukunftswitness 238/238, `npm test` 9.892/9.892 bei 0 offenen Handles, Browser 27/27, `npm run docs:evidence` gruen, Syntax-/JSON-/Diffchecks gruen. G-04 bis G-10 bleiben als Restrisiken unveraendert offen. Unabhaengiges Re-Review, Commit und Push stehen aus. |
| 2026-07-29 | Gesamtreview-Nachbesserung durch Claude re-reviewt | Status freigegeben; keine Blocker. G-01, G-02 und G-03 geschlossen und unabhaengig nachgemessen. G-03 ist an der Ursache behoben: statt einer spaeteren festen Jahreszahl leiten beide Roundtrips Zieljahr, Stichtag, Dateiname, CSV-Zeilen und Erwartungen aus dem abgeschlossenen Kalenderjahr des jeweiligen Laufzeitkontexts ab; `deriveCompletedCalendarYear` ist nachgeprueft `getFullYear() - 1` in Ortszeit, der Node-Test rechnet identisch, der Browsertest liest ueber `page.evaluate` aus dem Seitenkontext. Mit eingefrorener Uhr bleibt `npm test` an vier Zukunftsterminen (2027-01-01, 2027-06-15, 2030-05-05, 2035-12-31) konstant bei 9.892/9.892 - auch die Assertionszahl faellt nicht mehr ab. G-01: 65 Inventar-Findings deckungsgleich mit den 65 Registerzeilen, Slice-16-Teilmenge exakt 38, dreifach gebunden gegen Plan und feste Erwartungslisten. G-02: I-01 bis I-08 mit exaktem Plantitel und aufloesbarer Orakel-/Witness-/Gate-Kette. Zuordnungen stichprobenhaft fachlich geprueft (BAL-06 -> O-08, DAT-01 -> O-05, SIM-02 -> O-10, IMP-02 -> O-17, ENG-06 -> O-19), alle 22 Orakel belegt, 29 direkte Witness-Marker, keiner in einer Kommentarzeile. Fail-closed mit 13 Mutationen gegen die Plankopplung geprueft: 10 erkannt, 3 unbemerkt (Umbenennung zweier Endmarken und ein spaeteres Zweitvorkommen einer Startmarke, heute folgenlos). Scope eingehalten: ausschliesslich `tests/` und `docs/internal/`, `git diff --check` gruen. Nicht deklarierter Nebeneffekt: die Witness-Dateien steigen von 21 auf 29, die Slices 14 und 15 sind jetzt vertreten - G-08 damit weitgehend erledigt, `snapshot-archive.test.mjs` fehlt weiterhin. Neue Restrisiken H-1 (der Browsergate fiel in einem von vier Laeufen mit `page.waitForFunction: Timeout 30000ms` in `tests/browser-smoke.test.mjs:490` aus - vorbestehendes Profilverbund-Szenario, keine Regression, aber neun Befunde haengen jetzt an einem als "Node + Browser" ausgewiesenen Nachweis), H-2 (die zwei `Array.isArray`-Assertionen in `assertTraceabilityOwner` koennen an ihren Aufrufstellen nicht fehlschlagen, weil dort `x || []` uebergeben wird; 20 Findings fuehren kein `oracleIds`, 39 kein `witnesses`, sieben von acht Invarianten kein `witnesses`), H-3 (der Contract prueft Existenz und Aufloesbarkeit, nicht fachliche Angemessenheit; QA-01 verweist auf alle 22 Orakel zugleich und ist damit nicht verletzbar), H-4 (der Arbeitsplan ist ein Testfixture geworden; eine gewollte Umformatierung macht `npm test` rot mit einer Meldung, die auf ein Fixture zeigt), H-5 (`SuiteDataIntegrityTraceabilityV1` ist unter unveraendertem Versionsnamen ein groesserer Vertrag; fehlende Pflichtfelder erzeugen einen `TypeError` statt einer benannten Assertion), H-6 (der Zuwachs von 708 Assertionen entfaellt vollstaendig auf `suite-data-integration-contract.test.mjs` mit 358 -> 1.066 und misst Inventarbuchfuehrung, nicht Anwendungsverhalten) und H-7 (theoretisches Rennen am Jahreswechsel zwischen Testableitung und Produktaufruf, in fuenf Versuchen nicht reproduzierbar). Geprueft und verworfen: Tautologie der Datumsableitung, Zeitzonendivergenz, stille Assertionsverluste in der Zukunft, Marker in Kommentaren, Sammelverweise als Fuellmaterial, fehlende Witness-Dateien und Aenderungen ausserhalb des Testbereichs. Unveraendert offen bleiben G-04 bis G-07, G-09, G-10 und die mindestens 92 Restrisiken der Einzelslices. |
...
| T15-1 | Claude, Nutzer | Kalenderabhaengigkeit der Gesamtsuite | Nutzer entscheidet separaten datumssensitiven Doku-Gate; fuenfte Programmdatei genehmigt | technisch umgesetzt, im Re-Review freigegeben |
| S16-1, S16-2 | Claude/Gemini | Blocker Slice 16 | Bärenmarkt-Refill UI-Control wiederhergestellt, Fachkonzept-Tabelle vervollständigt | umgesetzt und im Re-Review durch Claude & Gemini freigegeben |
| S16-3 bis S16-10 | Claude/Gemini | Restrisiken Slice 16 | Contract-Test, Fail-Closed, Gate-Deklaration und Doku synchronisiert | umgesetzt und im Re-Review durch Claude & Gemini freigegeben |
