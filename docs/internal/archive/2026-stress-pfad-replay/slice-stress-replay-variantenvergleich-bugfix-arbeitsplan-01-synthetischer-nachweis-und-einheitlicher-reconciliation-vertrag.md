# Slice 01 – Synthetischer Nachweis und einheitlicher Reconciliation-Vertrag
...
Synthetischer Nachweis und einheitlicher Reconciliation-Vertrag
...
Erlaubter Scope: `app/simulator/stress-replay-contract.js`, `app/simulator/stress-replay-persistence.js`, `app/simulator/stress-replay-runner.js`, `app/simulator/stress-replay-ui.js`, `docs/internal/SLICE_STRESS_REPLAY_VARIANTENVERGLEICH_01_RECONCILIATION.md`, `docs/internal/slice-stress-replay-variantenvergleich-bugfix-arbeitsplan-01-synthetischer-nachweis-und-einheitlicher-reconciliation-vertrag.md`, `docs/internal/stress-replay-variantenvergleich-bugfix-implement-review-429c3799.md`, `tests/stress-replay-contract.test.mjs`, `tests/stress-replay-export.test.mjs`, `tests/stress-replay-persistence.test.mjs`, `tests/stress-replay-runner.test.mjs`, `tests/stress-replay-ui.test.mjs`

## Diff-Risiko inklusive Branch- und Statuscheck

Vor Coding geprüft: aktiver Branch `codex/stress-pfad-replay`. Der Status
enthielt ausschließlich dieses vom Orchestrator angelegte Slice-Dokument und
den unversionierten verwalteten Auditbericht. Diff-Risiko: hoch wegen des
persistierten und verschachtelt fingerprintgebundenen Source-Identity-
Vertrags. V1 darf weder umgedeutet noch automatisch migriert werden.
...
Der synthetische Red-State wurde vor Produktcodeänderungen bestätigt und in
`SLICE_STRESS_REPLAY_VARIANTENVERGLEICH_01_RECONCILIATION.md` protokolliert.
Source Identity V2 speichert eine begrenzte, feldpräsenztreue und kompakt
kanonisch kodierte Reconciliation-Projektion. Direkte Zeilen und V2 verwenden
denselben toleranzgebundenen Runner-Abgleich. V1 bleibt lesbar, ist aber mit
`source_identity_refix_required` nur inspizierbar; die UI fordert zum neuen
Fixieren des Stresspfads auf.

## Ausgeführte Validierung mit Ergebnis

Implementer-fokussiert erfolgreich: Contract 109/109, Runner 68/68,
Persistence 67/67, Export 27/27 und UI 99/99 Assertions. Außerdem war
der gezielte bestehende 60-Jahres-E2E-Lauf mit 91/91 Assertions erfolgreich;
sein Export blieb mit 109.472 Byte unter dem unveränderten relativen Budget
von 110.655 Byte. `git diff --check` war ohne Befund. Die autoritative Matrix
...
Restrisiko sind neue Kombinationen optionaler Source-Log-Felder über lange
Horizonte. Die V2-Allowlist, Feldpräsenztests, ein maximaler 60-Zeilen-Fall
und die fail-closed Fingerprint- und Pfadbindung begrenzen dieses Risiko.
Siehe zusätzlich Findings-Lebenszyklus.
