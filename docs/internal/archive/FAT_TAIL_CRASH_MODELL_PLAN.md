...
| `SLICE_TAIL_RISK_04_WORKER_METRICS.md` | freigegeben | `codex/fat-tail-crash-modell` | Tail-Risk-Totals werden log-unabhaengig pro Run gesammelt, ueber Worker-/Chunk-Merge addiert und als `aggregatedResults.extraKPI.tailRisk` bereitgestellt. Worker-aehnliche Full-vs-Split-Paritaet ist fuer aktiven Tail-Risk abgesichert; echter Worker-Entrypoint bleibt transfer- und payloadstabil. |
| `SLICE_TAIL_RISK_05_UI_DOCS.md` | implementiert, Review ausstehend | `codex/fat-tail-crash-modell` | Simulator-UI bietet ein bewusstes Tail-Risk-Opt-in mit Warnhinweis und Contract-validierten Parametern. Ergebnis-KPIs zeigen aktive/applizierte Tail-Risk-Runs/Jahre und historische Krisen-Skips nur bei aktivem Overlay. Scenario-Log-JSON/CSV exportiert Tail-Felder ueber die unverkuerzte Row-Struktur. README, TECHNICAL und Simulator-Modul-README dokumentieren Optionalitaet, Anti-Doppelpessimismus und Standard-vs.-Tail-Risk-Vergleich. |
