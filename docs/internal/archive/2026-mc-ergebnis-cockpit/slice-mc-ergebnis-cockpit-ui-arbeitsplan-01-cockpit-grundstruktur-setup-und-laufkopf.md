...
Cockpit-Grundstruktur, Setup und Laufkopf
...
Erlaubter Scope: `Simulator.html`, `app/simulator/mc-result-cockpit.js`, `app/simulator/monte-carlo-ui.js`, `app/simulator/simulator-main-init.js`, `docs/internal/mc-ergebnis-cockpit-ui-implement-review-5d8d7864.md`, `docs/internal/slice-mc-ergebnis-cockpit-ui-arbeitsplan-01-cockpit-grundstruktur-setup-und-laufkopf.md`, `simulator.css`, `tests/browser-smoke.test.mjs`, `tests/mc-result-cockpit.test.mjs`

## Diff-Risiko inklusive Branch- und Statuscheck

- Branch vor Umsetzung geprüft: `feature/mc-ergebnis-cockpit` entspricht dem Ziel-Branch.
- Ausgangsstatus: ausschließlich die vom Orchestrator angelegten, unversionierten Slice-/Audit-Dokumente; keine fremden Codeänderungen.
- Diff-Risiko: mittel. Die bestehenden MC-Knoten werden nur gruppiert beziehungsweise in die fachliche Reihenfolge verschoben; Engine-, Runner-, Persistenz- und Exportverträge bleiben unverändert.
- Produktiver Umfang: vier geänderte und ein neues Programm-/Konfigurationsartefakt; damit unter der Stopgrenze.

## Geplante Tests

- Fokussierter Modul-/DOM-Vertrag: `node tests/run-single.mjs tests/mc-result-cockpit.test.mjs`
- Bestehender MC-Parameter-/Lifecycle-Vertrag: `node tests/run-single.mjs tests/monte-carlo-parameters.test.mjs`
- Bestehende UI-Orchestrierung: `node tests/run-single.mjs tests/simulator-ui-orchestration.test.mjs`
- Browser-Smoke-Erweiterung für Maus, Enter, Space, Lifecycle und delegierten Start; autoritative Ausführung durch den Orchestrator.

## Durchgeführte Änderungen

- Native, initial offene Setup-Disclosure um die vier bestehenden Parametergruppen ergänzt.
- Reine Anzeigezusammenfassung für Runzahl, Dauer, Methodenlabel und Seed umgesetzt; Aktualisierung ohne Normalisierung oder Persistenz.
- Laufkopf mit weiterhin sichtbaren Status-, Progress-, Fehler-, Abbruch- und Exportknoten aufgebaut.
- „Neu rechnen“ delegiert genau an `#mcButton` und spiegelt dessen Disabled-/Busy-Zustand.
- Erfolgs-Lifecycle schließt das Setup; Fehler-, Abbruch- und sonstige Laufzustände verändern den Disclosure-Zustand nicht.
- Ergebniscontainer vor die Replay-Vertiefung verschoben; bestehende IDs und Rendererziele bleiben eindeutig.
- Cockpit-Initialisierung einmalig in den Simulator-Bootstrap eingebunden.

## Ausgeführte Validierung mit Ergebnis

- `node tests/run-single.mjs tests/mc-result-cockpit.test.mjs`: 29/29 Assertions erfolgreich.
- `node tests/run-single.mjs tests/monte-carlo-parameters.test.mjs`: 86/86 Assertions erfolgreich.
- `node tests/run-single.mjs tests/simulator-ui-orchestration.test.mjs`: 83/83 Assertions erfolgreich.
- `git diff --check`: erfolgreich.
- Die vollständige Browser-/Repository-Matrix wurde vertragsgemäß nicht im Agentenprozess ausgeführt; sie bleibt der Orchestrator-Validierung vorbehalten.
...
- Reale Browserausführung der ergänzten Tastatur- und Lifecycle-Smokes steht bis zur Orchestrator-Matrix aus.
- Spätere Slices müssen den zentralen Cockpit-Pfad erweitern, ohne Haupttab-Klassen oder bestehende Lifecycle-Hooks zu duplizieren.
