...
* `BALANCE_IMPORT_INPUT_SCHEMA_V2` inventarisiert alle importierbaren Top-Level-Eingaben mit Typ, Requiredness, Bounds und Enums; verschachtelte `decumulation`-, `healthBucket`- und Tranchenwerte besitzen eigene Validatoren. Unbekannte Felder, widerspruechliche CAPE-/Health-Bucket-Aliase und inkonsistente Runway-/Mindest-Flex-Grenzen blockieren vor DOM und Persistenz. `balance-state` V1 besitzt einen benannten Aufwaertsmigrator, der unter anderem `targetEq: 0`, `rebalBand: 0`, alte Prozentschreibweisen und die fruehere CSV-Untergrenzenrepraesentation erhaelt.
