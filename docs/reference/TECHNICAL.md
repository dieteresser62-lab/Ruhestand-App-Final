...
**Dokumentstand:** 2026-07-27; Codeabgleich bis zur Re-Review-Nachbesserung von Suite-Datenintegritaet Slice 02 auf dem lokalen Feature-Branch.
...
4. **`engine/transactions/TransactionEngine.mjs`** – leitet Ziel-Liquidität ab, steuert Puffer-Schutz und führt **Gap-basiertes Surplus-Rebalancing** (Investition nur bis Ziel-Allokation) durch.
   ...
   - `types/liquidity-runway-contract.js` definiert den kanonischen Nutzervertrag `liquidityRunwayYears` mit Default 5, Bereich 1 bis 10 und Schritt 0,5. `transaction-utils.mjs` leitet daraus das exakte Liquiditaetsziel als effektiven Nettojahresbedarf mal Zielmonate sowie die Engine-interne harte Mindest-Policy `min(Zielmonate, 24)` ab. Ziel und Jahresend-KPI werden nicht geglaettet; die finalen Runway-Kennzahlen verwenden die operative Liquiditaet nach Jahrestransaktionen und Auszahlung. Die neuen Felder `runway_post_payout_end_of_year_months`, `liq_post_payout_end_of_year` und in der Ansparphase `liq_post_accumulation_end_of_year` tragen diese Semantik explizit. `safety_runway_post_months` und `liqEnd` bleiben rueckwaertskompatibel bei ihrer bisherigen Bedeutung.
...
* Die Slice-13-Integrationsreferenz
  `tests/fixtures/backtest-data-integration-slice-13-v1.json` verbindet sechs
