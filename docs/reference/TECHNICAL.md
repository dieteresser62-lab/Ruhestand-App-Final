...
* `app/simulator/stress-replay-ui.js` und `stress-replay-renderer.js` – koordinieren Fixieren, Ersetzen, Verwerfen, Import/Export, Varianteneditor, Banner, Vergleichs- und Jahrestabellen. Der Haupteditor zeigt Name und die drei Bedarfsfelder mit getrennten EUR-Baselinewerten; die 17 bisherigen Parameter liegen in einem nicht persistierten, tastaturbedienbaren Expertenbereich. Ein expliziter Vergleichszustand trennt `idle`, `success` und `error`; Fehlercode und sanitizierte Meldung bleiben alert-semantisch sichtbar, und erfolgreiche Workspace-/Variantenmutationen werden nicht als erfolgreiche Vergleichsrechnung ausgegeben. Ein controllerweiter Busy-Vertrag blockiert konkurrierende Mutationen und Aktionen bis zum Abschluss der jeweiligen Promise. Inkompatible importierte Workspaces bleiben nur lesbar und koennen nicht ausgefuehrt werden.
