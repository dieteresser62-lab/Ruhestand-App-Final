...
* Sweep-Laufzeit: Der nominelle Aufwand ist Kombinationen × eigene Sweep-Laufzahl × maximale Dauer in Jahren. Bei mehr als 5.000.000 Laufjahren wird vor dem Start bestaetigt. `cancelParameterSweep()` bricht die aktive Generation ab, beendet ihren Worker-Pool und stoppt den seriellen Runner an der naechsten Yield-Grenze. Nur vollstaendige Ergebnisse mit erfolgreich gerenderter Heatmap werden gemeinsam mit `window.sweepExecution` veroeffentlicht; Abbruch und Fehler bewahren das vorherige Ergebnis. Der Fortschritt erreicht 100 % nur nach erfolgreicher Veroeffentlichung.
