...
4. **`engine/transactions/TransactionEngine.mjs`** – leitet Ziel-Liquidität ab, steuert Puffer-Schutz und führt **Gap-basiertes Surplus-Rebalancing** (Investition nur bis Ziel-Allokation) durch.
   ...
   - `types/liquidity-runway-contract.js` definiert den kanonischen Nutzervertrag `liquidityRunwayYears` mit Default 5, Bereich 1 bis 10 und Schritt 0,5. Nach `determineSpending()` gilt `netRunwayTarget = finalQuantizedNetPortfolioWithdrawal * targetMonths / 12`. Separat gilt `grossEmergencyBuffer = (floorBedarf + flexBedarf) * minCashBufferMonths / 12`; `minCashBufferMonths` ist von 0 bis 12 konfigurierbar und hat Default 2. `transaction-utils.mjs` rundet `max(netRunwayTarget, grossEmergencyBuffer)` auf volle 100 EUR auf. Die Engine-interne harte Mindest-Policy `hardMinimumMonths = min(targetMonths, 24)` ist nicht der Brutto-Puffer. Ziel und Jahresend-KPI werden nicht geglaettet; die operativen Runway-Kennzahlen verwenden die final geplante Entnahme und die Liquiditaet ihrer benannten Phase. `safety_runway_pre_months` bezeichnet den post-policy/pre-transaction Wert, `safety_runway_after_transaction_before_payout_months` denselben Nenner nach der Transaktion. Das historisch benannte `safety_runway_post_months` transportiert dagegen den post-transaction Dynamic-Flex-Safety-Wert auf Rohbedarfsbasis; `dynamicFlexSafetyRunwayBasis = pre_policy_annual_net_need` macht die Abweichung explizit. `runway_post_payout_end_of_year_months`, `liq_post_payout_end_of_year`, in der Ansparphase `liq_post_accumulation_end_of_year` und das rueckwaertskompatible `liqEnd` bleiben davon getrennt.
