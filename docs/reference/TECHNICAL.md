...
* `app/simulator/stress-replay-contract.js`, `stress-replay-path-materializer.js`, `stress-replay-runner.js` und `stress-replay-variant.js` – versionieren den fixierten exogenen MC-Pfad, materialisieren den opt-in Capture eines absoluten Run-Index, fuehren ihn RNG-frei mit Baseline oder Whitelist-Patch aus und validieren Fingerprints, Groessen sowie Modellgrenzen fail-closed.
* `app/simulator/stress-replay-transactions.js` und `stress-replay-comparison.js` – projizieren additive strukturierte Transaktionsdiagnostik, KPI-Deltas und erste Abweichungsmarker. Das Ergebnis bleibt ein gepaarter Gegenfakt auf genau einem Pfad; allgemeines Strategie-Ranking ist vertraglich gesperrt.
* `app/simulator/stress-replay-persistence.js` und `stress-replay-export.js` – speichern genau einen lokalen Workspace unter `sim.stressReplay.active.v1` und schreiben/lesen `StressReplayComparisonExportV1`. Reproduzierbare Jahresresultate werden nicht unnoetig persistiert; Pfad und Envelope besitzen 1-MiB-/2-MiB-Grenzen. Der Replay-Key ist von allgemeinen Snapshot-Capture-/Restore-Pfaden ausgeschlossen.
* `app/simulator/stress-replay-ui.js` und `stress-replay-renderer.js` – koordinieren Fixieren, Ersetzen, Verwerfen, Import/Export, Varianteneditor, Banner, Vergleichs- und Jahrestabellen. Inkompatible importierte Workspaces bleiben nur lesbar und koennen nicht ausgefuehrt werden.
...
* `npm run test:browser` fuehrt isolierte Playwright-Contexts fuer alle HTML-Einstiege sowie Balance-Contracts zu Profilabwahl/Reload, Engine-Mismatch, Jahres-Preflight, Doppelklick/Einmal-Commit, Import-Reject, Markt-CSV-Provenienz/Boolean-Roundtrip und korrupter Ausgabenpersistenz aus. Der Simulatorfall wartet auf fachliche Statuscodes statt Zeitpausen und prueft UI/Raw-Reconciliation, Cohorts, Caption/Headers/Fokus, JSON/CSV ohne HTML, Detailtoggle-Fingerprint, Falscheingaben, Datenluecke, Ruin, `technical_error` und Realbestands-Non-Mutation. Inflation, Yahoo-Proxy und CAPE werden deterministisch geroutet; andere externe Requests sind blockiert.
  * Der Stress-Replay-Durchstich liegt in `tests/stress-replay-e2e.test.mjs`: opt-in MC-Capture, 60-Jahres-Materialisierung, Baseline-Reconciliation, Variantenvergleich sowie Export/Import werden mit der echten Engine verkettet. Die versionierte Messfixture `tests/fixtures/stress-replay-performance-baseline-v1.json` dokumentiert Referenzumgebung, Einzelmessungen, Exportgroesse und das relative Medianbudget. Auf der Referenzumgebung wurden fuer Baseline plus eine Alternative 435,858 ms Median und 88.524 Byte Exportgroesse gemessen; das Gate verwendet den Faktor 4 mit einem absoluten 250-ms-Rauschboden. Die fruehere unbelegte Forderung `p95 < 50 ms fuer 240 Jahreslaeufe` ist kein Vertrag.
