...
* `app/simulator/mc-result-cockpit.js` – alleinige UI-State-Grenze fuer Setup-Zusammenfassung, Laufkopf und die fuenf MC-Ergebnisansichten. Das Modul initialisiert dokumentbezogen idempotent, verwaltet ARIA-Tabzustand und aktiviert fuer die bidirektionalen Fokuswege zwischen Szenario-Logs und Replay zuerst das zugehoerige sichtbare Panel. Der zweite „Neu rechnen“-Button delegiert an den kanonischen `#mcButton`; er ist kein eigener Laufpfad.
...
* `app/simulator/simulator-results.js` – Aggregiert MC-Ausgaben und delegiert an `results-metrics.js` / `results-renderers.js` / `results-formatting.js`.
  `displayMonteCarloResults()` ersetzt bei jedem Lauf nur den Inhalt des stabilen `#scenarioSelector` im Logs-Panel; der dynamische `#scenarioSelect` wird nicht als dauerhafter Navigationsknoten behandelt. Derselbe Auswahl-/Resetpfad projiziert Label, Endvermoegen beziehungsweise `FAILED` und Pflegemarkierung lesend in Replay-Schritt 1, ohne einen zweiten Auswahlzustand einzufuehren. Die Ergebnisprojektion aktiviert nach Erfolg den Cockpit-Ueberblick und blendet keine inaktive Ansicht eigenmaechtig ein.
...
* `app/simulator/auto_optimize_ui.js` und `app/simulator/auto-optimize-{presets,param-meta,config-ui,renderer,apply}.js` – Auto-Optimize UI-Fassade, Preset-Konfigurationen, Config-Parsing, Ergebnis-Rendering und Champion-Apply-Flow (1-6 dynamische Parameter).
...
Balance-JSON- und Markt-CSV-Import seit Hardening-Slices 08 und 12:
...
* Die MC-Navigation verwendet ausschließlich `.mc-view-*` und damit weder die Klassen noch die Ereignissteuerung der vier Simulator-Haupttabs. `activateMonteCarloResultView()` ist der zentrale Aktivierungsweg fuer View-IDs und enthaltene Fokusziele; andere Module manipulieren `hidden`, `aria-selected` oder den Roving-Tabindex der Ergebnisansichten nicht direkt.
* `#scenarioSelector` ist ein stabiler Einzelknoten in „Szenario-Logs“ unmittelbar vor der gesteuerten Logausgabe. Ein neuer MC-Lauf darf seinen dynamischen Select-Inhalt ersetzen, aber weder den Replay-Workspace noch die Navigation neu erzeugen. Replay-Schritt 1 zeigt dieselbe aktuelle Auswahl nur lesend. Beide Rueckverweise aktivieren vor dem Fokus das Zielpanel; Logs fokussieren den sichtbaren Replay-Fixier-/Projektionsbereich, Replay fokussiert den dynamischen Select oder vor einem Lauf den sichtbaren Logs-Rueckverweis.
* `#mcSetupDisclosure` bleibt ein natives `details` ohne JavaScript-Offenzustand. Das Summary zeigt per `[open]` und CSS-Chevron „Setup ausblenden“ oder „Setup bearbeiten“; der erfolgreiche Lauf schliesst es weiterhin, waehrend Eingabewerte und der besondere Fokusvertrag des eingeklappten `#mcButton` erhalten bleiben.
...
* `simulator.css` ist die alleinige Drucksteuerung: Setup-Disclosure, alle fuenf MC-Panels, Replay-Diagnosedetails und alle drei Vergleichssektionen werden unabhaengig vom Bildschirmzustand sichtbar. Tablisten, reine Umschaltaktionen und Sticky-Verhalten entfallen; feste Hoehen und abschneidendes Overflow sind deaktiviert. Der eindeutige `#print-footer` bleibt am Ende des MC-Bereichs. Es gibt keinen JavaScript-Print-State und keine Aenderung fachlicher Daten.
...
* **Dynamische Parameter-UI:** Unterstützt 1-6 frei konfigurierbare Optimierungsparameter mit individuellen Bereichen.
...
interaktive Sweep besitzt genau sieben Dimensionen mit geprueftem kanonischem
