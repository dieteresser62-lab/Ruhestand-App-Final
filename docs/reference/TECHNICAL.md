...
* `app/simulator/mc-result-cockpit.js` – alleinige UI-State-Grenze fuer Setup-Zusammenfassung, Laufkopf und die fuenf MC-Ergebnisansichten. Das Modul initialisiert dokumentbezogen idempotent, verwaltet ARIA-Tabzustand und aktiviert fuer programmatische Fokusziele zuerst das zugehoerige sichtbare Panel. Der zweite „Neu rechnen“-Button delegiert an den kanonischen `#mcButton`; er ist kein eigener Laufpfad.
...
* `app/simulator/simulator-results.js` – Aggregiert MC-Ausgaben und delegiert an `results-metrics.js` / `results-renderers.js` / `results-formatting.js`.
  `displayMonteCarloResults()` ersetzt bei jedem Lauf nur den Inhalt des stabilen `#scenarioSelector`; der dynamische `#scenarioSelect` wird nicht als dauerhafter Navigationsknoten behandelt. Die Ergebnisprojektion aktiviert nach Erfolg den Cockpit-Ueberblick und blendet keine inaktive Ansicht eigenmaechtig ein.
...
* `#scenarioSelector` ist ein stabiler Einzelknoten im Replay-Schritt „Lauf waehlen“. Ein neuer MC-Lauf darf seinen dynamischen Select-Inhalt ersetzen, aber weder den Replay-Workspace noch die Navigation neu erzeugen. Der Rueckverweis aus den Szenario-Logs aktiviert dieselbe Replay-Ansicht und fokussiert den vorhandenen Selektor.
