...
**Dokumentstand:** 2026-08-07 (Abschlusshaertung Slices 01 bis 03 lokal abgenommen; Claude-Findings zum append-only Reconcile-Cashstatus aus Slice 04 technisch nachgebessert, externes Re-Review ausstehend)
**Inhaltlicher Codeabgleich:** Architekturabschnitt B, Monte-Carlo-Fachkonzept C.3, Backtest-Fachkonzept C.8 sowie Rechenkonventions- und Modellgrenzen gegen Slice-03-Commit `68e9e0f` und den lokalen Implementierungsstand von Abschlusshaertung Slice 04
**Reproduzierbarer Inventarstand:** Commit `68e9e0f` plus lokaler, extern noch nicht freigegebener Slice-04-Arbeitsstand; Ermittlungsweg siehe Release-Checkliste
...
Balance und Simulator sind schreibfrei gegen den Realbestand. Erst eine tatsaechliche Broker-Ausfuehrung darf im Profil-Assets-Manager nach schreibfreier Vorschau und separater Bestaetigung reconciliert werden. Der Write umfasst Live-Lot, profilgebundenes Lot und datensparsamen Auditverlauf in einem Flush; identische Wiederholungen sind No-ops. Der Nettoerloes wird nie automatisch als freie Liquiditaet gebucht. Neue Verkaeufe bleiben standardmaessig `pending_manual_posting`; nach der manuellen Cashaktualisierung bestaetigt ein append-only Folgeereignis Zielverkauf, unveraenderten Nettoerloes und Cashstand. Tippfehler werden ueber eine lineare, revisionssichere Korrekturkette berichtigt, ohne Verkauf oder fruehere Nachweise zu mutieren. Die vollstaendige Modulgrenze steht in `TRANCHEN_MODULES_README.md`.
...
| MS-04 Verkauf/Reconciliation | Produkt-Owner: Nutzer; Accounting-Review unbesetzt | bestaetigte Stuecke, Nettoerloes, Gebuehr und append-only Cashnachweis/-korrektur in EUR; Contractstand 2026-08-07 | bestaetigte Lots werden entfernt; Verkaufserloes wird nicht automatisch gebucht; Cashstand bleibt eine explizite Nutzereingabe | ja, Reconciliation-, Persistenz- und Browser-Vertraege | teilweise, operativer Ablauf dokumentiert | nein | 2026-10-31 |
...
| D-15 Verkaufserloes | Reconciliation entfernt die bestaetigten Stuecke und protokolliert Nettoerloes/Gebuehr, erhoeht aber nicht automatisch die freie Liquiditaet. Der Default bleibt offen, bis der Nutzer den Cashstand manuell aktualisiert und append-only bestaetigt; ein bereits beruecksichtigter Erloes kann beim Verkaufscommit explizit dokumentiert und ein falscher Cashstand append-only korrigiert werden. | **entschieden und technisch umgesetzt** in Abschlusshaertung Slice 04; externe Accounting-Validierung des persoenlichen Ablaufs bleibt MS-04. |
