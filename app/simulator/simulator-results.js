...
export function getWorstRunColumnDefinitions(opts = {}) {
    ...
    const detailCols = options.logDetailLevel === 'detailed' ? [
        ...
        { key: 'health_bucket_end', header: 'HBEnd', width: 9, fmt: formatCurrencyShortLog },
        { key: 'health_bucket_target_inflation_adjusted', header: 'HBZiel', width: 9, fmt: formatCurrencyShortLog },
        {
            key: 'health_bucket_real_coverage_pct',
            header: 'HBDeck%',
            width: 8,
            fmt: v => Number.isFinite(Number(v)) ? formatPercentValue(Number(v), { fractionDigits: 0, invalid: '' }) : ''
        },
        { key: 'health_bucket_target_gap', header: 'HBLücke', width: 9, fmt: formatCurrencyShortLog },
        { key: 'health_bucket_triggered', header: 'HBTrig', width: 6, fmt: v => (v ? 'aktiv' : '') },
        { key: 'health_bucket_warning', header: 'HBWarn', width: 18, fmt: v => (v || '').substring(0, 18) },
