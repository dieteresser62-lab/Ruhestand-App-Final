...
export function buildBacktestColumnDefinitions(detailLevel = 'normal', options = {}) {
    ...
    if (isDetailed) {
        columns.push(
            ...
            { header: 'HBEnd', width: 9, key: 'row.health_bucket_end', valueFormatter: v => formatCurrencyShortLog(v), align: 'right' },
            { header: 'HBZiel', width: 9, key: 'row.health_bucket_target_inflation_adjusted', valueFormatter: v => formatCurrencyShortLog(v), align: 'right' },
            { header: 'HBDeck%', width: 8, key: 'row.health_bucket_real_coverage_pct', valueFormatter: v => Number.isFinite(Number(v)) ? formatPercentValue(Number(v), { fractionDigits: 0, invalid: '' }) : '', align: 'right' },
            { header: 'HBLücke', width: 9, key: 'row.health_bucket_target_gap', valueFormatter: v => formatCurrencyShortLog(v), align: 'right' },
            { header: 'HBTrig', width: 6, key: 'row.health_bucket_triggered', valueFormatter: v => (v ? 'aktiv' : ''), align: 'left' },
            { header: 'HBWarn', width: 18, key: 'row.health_bucket_warning', valueFormatter: v => (v || '').substring(0, 18), align: 'left' },
            ...
            { header: 'RunRaw', width: 7, key: 'row.RunwayTargetRawMonths', valueFormatter: v => (Number.isFinite(v) ? `${Number(v).toFixed(0)}M` : ''), align: 'right' },
            { header: 'RunMin', width: 7, key: 'row.RunwayTargetHardMinMonths', valueFormatter: v => (Number.isFinite(v) ? `${Number(v).toFixed(0)}M` : ''), align: 'right' },
            { header: 'RunFb', width: 5, key: 'row.RunwayTargetSmoothingFallback', valueFormatter: v => (v ? 'ja' : ''), align: 'left' },
