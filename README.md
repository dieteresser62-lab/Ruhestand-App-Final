...
* **Kanonischer Liquiditaets-Runway:** `liquidityRunwayYears` (Default 5 Jahre, zulaessig 1 bis 10 in 0,5er-Schritten) steuert das Ziel aus effektivem Nettojahresbedarf mal Monaten. Runway-Ziel und Jahresend-KPI werden nicht geglaettet; Diagnose, Warnungen und Backtest-Mindestdeckung verwenden den finalen Bestand nach Jahrestransaktionen und Auszahlung. `runway_post_payout_end_of_year_months` und `liq_post_payout_end_of_year` benennen diesen neuen Jahresendstand explizit; bestehende Felder wie `safety_runway_post_months` und `liqEnd` behalten ihre bisherige Semantik. Der Engine-interne harte Mindestwert ist eine daraus abgeleitete Policy, keine zweite Nutzereingabe.
