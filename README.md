...
* **Stress-Pfad-Replay:** Ein ausgewaehltes Monte-Carlo-Szenario kann als vollstaendiger exogener Pfad fixiert und mit der unveraenderten Baseline sowie bis zu drei erlaubten Strategievarianten erneut gerechnet werden. Der Vergleich gilt ausschliesslich fuer diesen einen Pfad und ist weder allgemeines Ranking noch Finanzempfehlung. Die Variantenfelder `maxSkimPctOfEq` und `maxBearRefillPctOfEq` akzeptieren fail-closed 0–50 bzw. 0–70 Prozent; Assetallokation, Mindest-Flex, reale Profile und Tranchen sind keine Variantenfelder. Nach Ruin setzt der fixierte Pfad den kanonischen Marktstatus fort; `ruin` und `all_dead` besitzen symmetrische Terminalzeilen, wobei das Todesjahr finanziell nicht ausgewertet wird. Unbekannte Transaktionswerte bleiben bis in die Breakdown-Summen explizit fehlend. Pfad, unabhaengige Herkunftsidentitaet, Baseline und Varianten werden lokal als ein aktiver Arbeitsstand transaktional gespeichert; ein neuer Pfad ersetzt ihn erst nach Bestaetigung. Versionierter JSON-Export/-Import erhaelt die Herkunftsidentitaet, Fingerprints und Groessenlimits; Daten-/Engine-Mismatch erlaubt nur die Inspektion. Waehrend asynchroner Aktionen sperrt ein gemeinsamer Busy-Zustand alle konkurrierenden Replay-Aktionen. KPI-Deltas kennzeichnen Jahresdifferenzen und Prozentpunkte getrennt.
