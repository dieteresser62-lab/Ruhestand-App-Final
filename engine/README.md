...
Die Berechnungs-Engine wurde aus dem historischen Monolithen extrahiert und besteht nun aus klar getrennten ES-Modulen. `build-engine.mjs` ruft einen einfachen esbuild-Bundle-Lauf (oder einen Modul-Fallback) auf und erzeugt `engine.js`, das weiterhin den globalen Export `EngineAPI` bereitstellt.
...
`EngineAPI` stellt die Methoden `getVersion()`, `getConfig()`, `analyzeMarket()`, `calculateTargetLiquidity()` und `simulateSingleYear()` bereit. Alle Aufrufer nutzen den einheitlichen `EngineAPI`-Export.
...
3. Balance-App und Simulator testen – beide nutzen denselben gebündelten Einstiegspunkt `EngineAPI` bzw. den Modul-Fallback.
