...
Das Browser-Gate nutzt Playwright mit einem vom Test verwalteten lokalen HTTP-Server. Jeder Fall erhaelt einen isolierten Browser-Context und eine eigene Storage-Baseline. Neben den zentralen Einstiegspunkten (`index.html`, `Balance.html`, `Simulator.html`, `depot-tranchen-manager.html`, `Handbuch.html`) prueft es in `Balance.html` Profilabwahl nach Reload, Engine-Mismatch, mutationsfreien Jahres-Preflight, sichtbare korrupte Ausgaben, sichtbaren Import-Reject, einen Markt-CSV-Roundtrip mit periodengebundener Provenienz/`windowHigh`/sichtbarer gerichteter ATH-Untergrenze samt Anwendungsstatus/Boolean-Reload, einen Doppelklick mit genau einem Jahrescommit und Recovery-Snapshot sowie die 3-Bucket-Bear-Diagnose aus der realen Engine-Rendite. Die Simulatorfaelle warten auf fachliche Statuswerte statt auf feste Millisekunden. Fuer das MC-Ergebnis-Cockpit prueft das Gate die fuenf vom Haupttab-System getrennten ARIA-Tabs, Roving-Tabindex und Pfeil/Home/End, den Setup-/Laufkopf-Lifecycle, den Szenario-Log-Rueckverweis, asynchrone Fokusreaktivierung und die drei rerenderfesten Replay-Vergleichsansichten. Die abschliessende Responsive-/Printmatrix ergaenzt diese Interaktionen um 320, 768, 900, 1280 und 1600 CSS-Pixel sowie um sichtbares Setup, alle MC-Panels, alle Vergleichssektionen und deaktivierte Navigation/Sticky-Positionierung im Druckmedium. Der Stress-Replay-Workflow prueft ausserdem den echten leeren Startzustand, das native deaktivierte Fieldset, die Fokusansicht aus Name plus drei Bedarfsfeldern, den initial geschlossenen Expertenbereich samt ARIA-/Werterhalt und die deutschsprachige Anzeige des echten Mindest-Flex-Relationsfehlers. Fixieren, priorisierte Kernaussage, Variantenvergleich und getrennte Texte fuer Jahres- und Prozentpunktdeltas bleiben abgedeckt; der Deferred-Promise- und Reentrancy-Nachweis des Busy-Vertrags liegt im separaten `stress-replay-ui.test.mjs`. Der Backtestfall reconciliiert sichtbare Periode, Outcome, Jahrinventar, exakte 10-%-Metrik, Pflegebucket und Cohort-Inventar mit Raw-JSON und deckt die Negativpfade ab. Inflation, Yahoo-Proxy und CAPE werden deterministisch geroutet; andere externe Requests werden blockiert. Das Gate ersetzt keine Node-Unit-Tests und ist fuer Browser-Regressionen ein explizit auszufuehrendes Pflicht-Gate neben `npm test`.
...
| `browser-smoke.test.mjs` | ~2670 | Playwright-Gate fuer HTML-Einstiege, MC-/Backtest-UI, A11y/Negativpfade sowie zentrale Balance-/Tranchenflows einschliesslich Reconcile-Pending, Seitenwechsel, Cashabschluss, append-only Korrektur, Retry und Legacy-Anzeige |
| `mc-result-cockpit.test.mjs` | ~280 | Fokussierter DOM-Contract fuer Setup-/Laufkopf-Lifecycle, idempotente Listener, fuenf ARIA-Ergebnisansichten, zentrale Zielaktivierung, Replay-Schrittstatus und delegierte Vergleichstabs |
...
| `stress-replay-e2e.test.mjs` | ~360 | Echter 60-Jahres-Durchstich ueber V2-Floor/Flex/Mindest-Flex bis Engine-Diagnostik, Nullwert-Export/Import, erneut ausgefuehrter V1-Roundtrip, Groessenlimit und getrenntes Performancebudget |
| `stress-replay-contract.test.mjs` | ~380 | Fail-closed V1-/V2-Schemas, Bedarfsrelation, Variantenbereiche 0–50/0–70, Terminalzeilen und unabhaengige Herkunftsidentitaet |
| `stress-replay-persistence.test.mjs` | ~370 | Transaktionales Speichern/Verwerfen, Readback, Rollbackfehler, Reload-/Import-Identitaet und Nur-Lesen-Kompatibilitaet |
| `stress-replay-transactions.test.mjs` | ~500 | Ausschliesslich belegte Transaktionsklassen, Capture-Paritaet sowie explizite Event-/Breakdown-Missingness und Summenabgleich |
| `stress-replay-ui.test.mjs` | ~740 | Controllerweiter Busy-/Reentrancy-Vertrag, Deferred-Promise-Rennen, Reload/Import sowie sichtbare Replay-Aktivierung vor asynchronen Fokuszielen |
| `stress-replay-renderer.test.mjs` | ~400 | Terminalzeilen, Missingness, priorisierte nicht empfehlende Kernaussagen, Vergleichs-Unteransichten und eindeutige Jahres-/Prozentpunkt-Deltatexte |
...
`fixtures/stress-replay-comparison-export-v1.json` ist das Golden-Orakel fuer
einen historischen Export mit ausschliesslich V1-Whitelistvarianten. Der
E2E-Test erzeugt zusaetzlich aus einem echten materialisierten Pfad einen
V1-Export, importiert ihn und fuehrt ihn mit Baselineuebernahme fuer die
fehlenden V2-Bedarfsblaetter erneut aus.
