...
(function(global) {
    ...
    const SpendingPlanner = {
      determineSpending(p) {
        const { lastState, market, inflatedBedarf, round5, runwayMonate, profil, depotwertGesamt, gesamtwert, renteJahr, input } = p;
        ...
        if (round5) {
            const ungerundeteFlexRate = inflatedBedarf.flex > 0 ? ((endgueltigeEntnahme - inflatedBedarf.floor) / inflatedBedarf.flex * 100) : 0;
            const gerundeteFlexRate = Math.round(ungerundeteFlexRate / 5) * 5;
            if (Math.abs(ungerundeteFlexRate - gerundeteFlexRate) > 0.1) {
                 endgueltigeEntnahme = inflatedBedarf.floor + (inflatedBedarf.flex * (gerundeteFlexRate / 100));
...
determineAction(p) {
    ...
    function _internal_calculateModel(input, lastState) {
        ...
        const { spendingResult, newState, diagnosis } = SpendingPlanner.determineSpending({ market, lastState, inflatedBedarf, round5: input.round5, runwayMonate: reichweiteMonate, profil, depotwertGesamt, gesamtwert, renteJahr, input });
...
const Ruhestandsmodell_v30_Adapter = {
    ...
    determineSpending: function({market, lastState, inflatedFloor, inflatedFlex, round5, runwayMonths, liquidNow, profile, depotValue, inputsCtx, totalWealth}) {
